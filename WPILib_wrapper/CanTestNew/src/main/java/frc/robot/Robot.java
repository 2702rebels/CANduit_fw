...
public class Robot extends TimedRobot {
  ...
  */
  //This uses the team manufacturer and device type by default. The device ID is 6 bits (0-63).
  ...
  public void robotPeriodic() {
    ...
    // Read back the value (should be incremented by 1 by the f/w)
    // Send the RTR Frame (Requesting 1 bytes of data)
    m_cangio.writeRTRFrame(1, apiId);
    
    // Read the response packet
    // readPacketNew returns true if a NEW packet has been received since last call
    // Give up after 10 tries  PHIL - the retries seem to be unnecessary
    for (int i = 0; i < 10; i++) {
      if (m_cangio.readPacketNew(apiId, m_data)) {
          if ((m_data.data[0] & 0xFF) != 0xA5)
          {
            System.out.printf("Unexpected REply: 0x%02X%n", m_data.data[0]);
          }
          //System.out.printf("Read: 0x%02X%n", m_data.data[0]); // Print the received byte array
          break;
      }
      else {
    ...
    //System.out.println("Failed - No reply found");
